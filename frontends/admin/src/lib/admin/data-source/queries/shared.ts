...
/**
 * Раздел без аналога в платформе: данные всегда демонстрационные, даже в режиме `api`.
 * Так интерфейс раздела остаётся рабочим и не бьётся о несуществующие эндпоинты.
 */
export async function mockOnly<T>(mockData: T | (() => T)): Promise<T> {
  const data = typeof mockData === "function" ? (mockData as () => T)() : mockData;
  return withMockDelay(data);
}

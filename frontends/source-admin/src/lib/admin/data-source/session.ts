...
/** Текущий проект оператора: bootstrap → cookie для скоупа запросов. */
async function resolveCurrentProject(token: string) {
  ...
  const payload = (await response.json()) as {
    data?: { current_project?: string | null; projects?: { key: string }[] };
  };
...
export async function signInOperator(
  ...
  if (shouldUseAdminApi()) {
    try {
      await resolveCurrentProject(token);
    } catch {

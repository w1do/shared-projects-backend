...
# Учётка оператора — та же переменная, что у auth-service в compose:
# смена ADMIN_PASSWORD в .env не должна ронять прогон на входе
ADMIN_EMAIL="${ADMIN_EMAIL:-root@example.com}"
ADMIN_PASSWORD="${ADMIN_PASSWORD:-secret-123}"
...
\$a = Cms\Auth\Domain\Models\Admin::query()->firstOrCreate(['email' => '$ADMIN_EMAIL'], ['name' => 'Root', 'password' => '$ADMIN_PASSWORD']);
...
echo "== sync cities directory"
$COMPOSE exec -T -w /var/www/apps/content-service content-service php artisan city:sync
...
TOKEN=$(curl -fsS -X POST "$BASE/api/admin/v1/auth/login" -H 'Content-Type: application/json' \
    -d "{\"email\":\"$ADMIN_EMAIL\",\"password\":\"$ADMIN_PASSWORD\"}" | jqr "['data']['token']")
AUTH="Authorization: Bearer $TOKEN"

echo "== project"
curl -fsS -X POST "$BASE/api/admin/v1/projects" -H "$AUTH" -H 'Content-Type: application/json' \
    -d "{\"key\":\"$SMOKE_PROJECT\",\"name\":\"Demo Site\"}" >/dev/null || true
for svc in content analytics pay; do
    curl -fsS -X PUT "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/services/$svc" -H "$AUTH" \
...
curl -fsS -X PUT "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/pay/settings" -H "$AUTH" \
    -H 'Content-Type: application/json' -d '{"provider":"manual"}' >/dev/null

PROJECT_ID=$(curl -fsS "$BASE/api/admin/v1/projects/$SMOKE_PROJECT" -H "$AUTH" | jqr "['data']['id']")

echo "== sync localization keys"
# Ключи реестра заводятся на проект: без явного id свежий проект команде не
# виден — она берёт проекты из локальных таблиц content-service
$COMPOSE exec -T -w /var/www/apps/content-service content-service \
    php artisan localize:sync --project="$PROJECT_ID"

PK=$(curl -fsS -X POST "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/api-keys" -H "$AUTH" \
    -H 'Content-Type: application/json' -d '{"type":"public"}' | jqr "['data']['key']")
SK=$(curl -fsS -X POST "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/api-keys" -H "$AUTH" \
    -H 'Content-Type: application/json' -d '{"type":"secret"}' | jqr "['data']['key']")
echo "keys: ${PK:0:12}… ${SK:0:12}…"

echo "== bootstrap"
curl -fsS "$BASE/api/admin/v1/bootstrap?project=$SMOKE_PROJECT" -H "$AUTH" \
...
POST_ID=$(curl -fsS -X POST "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/content/posts" -H "$AUTH" \
    -H 'Content-Type: application/json' -d "{\"title\":\"Hello Platform $RUN\",\"body\":\"First post\"}" | jqr "['data']['id']")
curl -fsS -X POST "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/content/posts/$POST_ID/status" -H "$AUTH" \
...
curl -fsS -X POST "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/pay/plans" -H "$AUTH" -H 'Content-Type: application/json' \
...
curl -fsS -X POST "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/pay/payments/$PAY_ID/confirm" -H "$AUTH" | jqr "['data']['status']"
...
ORG_ID=$(curl -fsS -X POST "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/pay/licensing/organizations" -H "$AUTH" \
    -H 'Content-Type: application/json' \
    -d "{\"name\":\"Smoke Org $RUN\",\"contact_first_name\":\"Ivan\",\"contact_last_name\":\"Petrov\",\"email\":\"org-$RUN@example.com\"}" \
    | jqr "['data']['id']")
LPLAN_ID=$(curl -fsS -X POST "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/pay/licensing/plans" -H "$AUTH" \
    -H 'Content-Type: application/json' -d "{\"code\":\"box-$RUN\",\"name\":\"Box\"}" | jqr "['data']['id']")
curl -fsS -X POST "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/pay/licensing/releases" -H "$AUTH" \
    -H 'Content-Type: application/json' \
    -d "{\"version\":\"1.0.$((RUN % 100000))\",\"train\":\"1.0\",\"repository\":\"crm/app-1.0\",\"released_at\":\"2026-01-10T00:00:00+00:00\"}" >/dev/null
LKEY=$(curl -fsS -X POST "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/pay/licensing/licenses" -H "$AUTH" \
    -H 'Content-Type: application/json' \
...
curl -fsS "$BASE/api/admin/v1/projects/$SMOKE_PROJECT/analytics/history/user:$PROJECT_ID:$USER_ID" -H "$AUTH" \

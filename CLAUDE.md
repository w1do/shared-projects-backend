...
## Новая фича — сразу в бутстрап и в переменные

- **Всё, что нужно окружению для работы фичи, добавляется в бутстрап тем же коммитом**: `infra/docker/entrypoint.sh` и `tools/smoke.sh` (его прогоняет `./tools/cms bootstrap`). Прод и чистый стенд поднимаются одной командой; шага «после выката зайди и допиши руками» не существует.
- **Права и роли руками не заводятся.** Право объявляется в `ServiceManifest` своего сервиса, системная роль — шаблоном в `cms-auth.system_roles`, а состояние всех проектов приводит команда `permissions:sync`. Она стоит в бутстрапе после публикации манифестов и печатает сводку: сколько прав добавлено и обновлено, какие осиротели.
- **Новая консольная команда, миграция, сид данных, провижининг внешнего хранилища — тоже часть бутстрапа**, а не устная инструкция в переписке. Не попало в бутстрап — на следующем окружении фичи нет.
- **Различия окружений — переменными.** URL, ключи, лимиты, включённые сервисы и прочее, что меняется между дев/прод, живёт переменными окружения с дефолтами в compose и читается через `config/*`. Хардкод значений в коде и «поправлю на проде вручную» запрещены.
- **Состав добавленного виден из бутстрапа и его сводки, а не из памяти автора.** Угадывать, что ещё нужно донастроить после выката, не должно быть нужно ни человеку, ни агенту.

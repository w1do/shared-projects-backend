...
- [ ] `ADMIN_EMAIL`, `ADMIN_PASSWORD` (при `APP_ENV=production` dev-пароль не принимается — оператор не будет создан)
- [ ] `ANALYTICS_IP_SALT`
- [ ] `GATEWAY_BIND=127.0.0.1` — иначе стек доступен по plain HTTP на публичном IP мимо TLS
- [ ] `MAIL_*` — без SMTP сброс пароля оператора не доходит до адресата (письма пишутся в лог)

Пароли хранилищ фиксируются при **первом** старте (в момент создания томов): позже
их смена в переменных не меняет пароль внутри БД — сервисы просто перестанут
подключаться. Менять пароль нужно внутри самой БД либо вместе с пересозданием тома.

Первый старт создаёт корневого оператора и стартовый проект (`INITIAL_PROJECT_KEY`,
по умолчанию `main`) со всеми включёнными сервисами — консоль сразу с разделами.
Повторные деплои ничего не пересоздают и не перезаписывают.

Медиа отдаются через gateway по пути `/media` (публичные URL строятся от `APP_URL`);
отдельный домен для MinIO нужен, только если задан свой `AWS_URL`.
...
## Версия работающего кода

Версия проставляется в образ на сборке: `APP_VERSION` → аргумент сборки → переменная
окружения контейнера. Сервис, его воркер и планировщик собираются из одного образа,
поэтому расхождение версий означает, что обработчик остался на прежнем коде.

```bash
./tools/cms deploy            # выкат: версия = короткий SHA коммита
./tools/cms deploy v1.4.0     # выкат с явной версией
./tools/cms version           # версия в каждом контейнере платформы
```

`/health` каждого сервиса отдаёт ту же версию: `{"service":…,"status":"ok","version":…}`.
Сборка без аргумента даёт `unknown` — это валидное значение, а не ошибка старта.

## Обновление и rollback

- **Обновление**: push в отслеживаемую ветку → redeploy сервиса в Dokploy (или локально
  `./tools/cms deploy`). Выкат поднимает сервисы вместе с воркерами и планировщиками
  одной операцией: долгоживущий обработчик иначе продолжает выполнять прежний код.
  Данные живут в именованных томах и переживают пересборку; бутстрап идемпотентен —
  повторный старт не создаёт дублей и не перезаписывает изменённые данные (включая
  пароль оператора).
